# pg_reindex
# Скрипт для переиндексации баз postgresql с помощью команды REINDEX.
# Version: 1.00
...
def reindex_database(pg_reindex_path, host, port, username, db_name, verbose):
    command = [
        pg_reindex_path,
        "--host", host,
        "--port", str(port),
    ...
    try:
        ...
        msg = f"Reindexing for database '{db_name}' completed successfully."
        log_message(msg)  # Записываем в лог краткую информацию
        if verbose:
            print(msg)
        return True
    except subprocess.CalledProcessError as e:
        msg = f"Error reindexing database '{db_name}': {e.output.decode() if hasattr(e, 'output') else str(e)}"
...
def main():
    ...
    # Выполнение переиндексации для каждой базы данных
    for db in databases:
        success = reindex_database(settings['pg_reindex_path'], settings['host'],
        ...
        if success:
            if settings.get('telegram_notifications_enabled'):
                send_telegram_notification(settings['telegram_script_path'], "r",
                                            f"{settings.get('prefix', '')} {host_name}. Reindexing for '{db}' completed successfully.")
        else:
            if settings.get('telegram_notifications_enabled'):
                send_telegram_notification(settings['telegram_script_path'], "a",
                                            f"{settings.get('prefix', '')} {host_name}. Reindexing for '{db}' failed.")
